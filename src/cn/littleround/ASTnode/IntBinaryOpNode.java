    ...
    public void updateType() {
        ...
        if (!(op1().type instanceof IntType) || !(op2().type instanceof IntType))
            if (op1().type == null || op2().type == null) {
                reportError("Semantic", "Expect int value between int operator, not null.");
            } else
                reportError("Semantic", "Expect int value between int operator, not "+op1().type.toString()+" and "+op2().type.toString()+".");
        type = Constants.INT;
    }
}
